...
| [deployment.md](deployment.md) | The live deployment — the frontend on Vercel, the backend on Render — its public URLs, the environment variables by name, how to deploy another instance and the order the two must be created in, exact-origin CORS, free-tier cold starts, and what horizontal scaling would require. |
