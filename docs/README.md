...
| [execution.md](execution.md) | Running code: the Run button and its shortcut, the five languages, the path from browser to backend to Judge0, why the credential stays server-side, every limit and what enforces it, how user-code errors differ from infrastructure errors, and the language-aware download. |
| [testing.md](testing.md) | What the 63 tests actually assert, case by case, on both sides; how the backend integration suite runs against a fake judge with no credential and no network; and what CI runs. |
